      ...
      class FormBuilderAdapter < ActiveAdmin::ObjectMapper::AbstractAdapter
        def filter(method, options = {})
          if method.present? && options[:as] ||= default_input_type(method)
            base.template.concat base.input(method, options)
          end
        end
        ...
        def default_input_type(method, options = {})
          if method =~ /_(eq|equals|cont|contains|start|starts_with|end|ends_with)\z/
            :string
          elsif base.klass._ransackers.key?(method.to_s)
            klass._ransackers[method.to_s].type
          elsif base.reflection_for(method) || base.polymorphic_foreign_type?(method)
            :select
          elsif column = base.column_for(method)
            case column.type
            when :date, :datetime
              :date_range
            when :string, :text
